...
def add_update_tab():
    ...
    if response.status_code == 200:
        ...
        st.error(f"Failed to fetch expenses, Status: {response.status_code}, Response: {response.text}")
        existing_expenses = []
    ...
    with st.form(key="expense_form"):
        ...
        if submit_button:
            ...
            req = requests.post(f"{API_URL}/expenses/{selected_date}", json=filtered_expenses)
